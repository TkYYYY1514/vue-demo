  ...
  <div class="terminal-interface">
    ...
    <div class="terminal-header">
      <div class="header-left">
        <div class="status-light gold"></div>
        <h1 class="terminal-title">核心标记库</h1>
      </div>
      <div class="header-right">
        <span class="system-time">{{ currentTime }}</span>
        <span class="record-count">核心记录: {{ favoriteNotes.length }}</span>
      </div>
    </div>
    ...
    <div class="data-grid">
      <div 
        ...
        :key="note.id"
        class="data-card priority"
        @click="editNote(note)"
      >
        <div class="priority-badge">核心</div>
        ...
        <div class="card-footer">
          <span class="timestamp">
            标记时间: {{ formatDate(note.updatedAt) }}
          </span>
      ...
      <!-- 空状态 -->
      <div v-if="favoriteNotes.length === 0" class="empty-terminal">
        <div class="empty-icon">⭐</div>
        <h3>暂无核心标记</h3>
        <p>尚未标记任何重要数据记录</p>
        <router-link to="/" class="terminal-btn secondary">
          返回数据档案
        </router-link>
      </div>
    </div>
    ...
    <div v-if="editingNote" class="editor-overlay" @click="closeEditor">
      <div class="editor-terminal" @click.stop>
        <div class="editor-header">
          <div class="editor-title">核心数据编辑器</div>
    ...
    <div v-if="showDeleteModal" class="editor-overlay" @click="hideDeleteConfirm">
      <div class="confirm-terminal" @click.stop>
        <div class="confirm-header warning">
          ...
          <div class="confirm-title">确认删除核心数据</div>
          <button @click="hideDeleteConfirm" class="close-terminal">⨯</button>
        </div>
        <div class="confirm-body">
          <div class="warning-message">
            <h3>即将删除核心数据记录</h3>
            <p class="record-info priority">"{{ noteToDelete?.title }}"</p>
            <p class="warning-text">此为核心标记数据，删除后将无法恢复，请谨慎操作！</p>
          </div>
...
.data-card.priority::before {
  background: linear-gradient(90deg, transparent, rgba(170, 140, 60, 0.1), transparent);
}
...
.data-card.priority:hover {
  border-color: #aa8c3c;
  box-shadow: 0 8px 25px rgba(170, 140, 60, 0.2);
}
...
.action-btn.favorite.active {
  background: rgba(170, 140, 60, 0.3);
  border-color: #aa8c3c;
  box-shadow: 0 0 8px rgba(170, 140, 60, 0.3);
}
...
.record-info.priority {
  background: rgba(170, 140, 60, 0.2);
  border-color: #aa8c3c;
  box-shadow: 0 0 10px rgba(170, 140, 60, 0.3);
}
